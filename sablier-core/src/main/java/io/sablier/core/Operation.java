...
/**
 * A read-only view of an asynchronous workload operation (start, stop) in progress on a
 * provider.
 *
 * <p>Providers complete operations from whatever mechanism they use internally (task polling,
 * long-poll, protocol events) via the corresponding {@link OperationHandle} — consumers only
 * ever see this read-only view.
 *
 * <p>{@link #await()} never throws for a {@link OperationStatus#FAILED} outcome; callers inspect
 * {@link #status()} and {@link #error()} themselves. {@link #await(Duration)} throws {@link
 * OperationException} only if the wait times out, since a timeout is a genuinely different,
...
public interface Operation {
    ...
    Optional<SablierException> error();
